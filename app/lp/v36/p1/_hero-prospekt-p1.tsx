...
/* ══ HERO «Проспект» — P1 «До героя» fork ═════════════════════════════════════
   P1's OWN hero (a verbatim fork of ../\_hero-prospekt's `HeroProspekt`, so the
   canonical file keeps ZERO diff). It starts identical and will DIVERGE to carry
   a compact proof strip INSIDE the composition — Danil's ask: the bank facts ride
   the hero, not a separate band below it (page renders `proofSlot={null}`). The
   monument safe-zone law + photo-ground contrast law are inherited unchanged.

   ── original header (inherited, ground/motion frozen) ────────────────────────
...
/* ── in-hero PROOF strip: the bank facts ride the hero's lower region (Danil's
   ask — no separate .proof band; the page renders proofSlot={null}). ONE figure
   (BANK_B_FMT, the bank stat) carries clear PRIMARY weight; the image count / the
   section count + the honest official-bank register stay subordinate on a compact
   glass chip (every numeral comes verbatim from ./copy). The chip's OWN dark
   fill + text-shadows GUARANTEE contrast over the photo ground regardless of the
   pixels behind it (≥4.5:1 body / ≥3:1 the large numeral) — a CSS-chain walk is
   invalid over a background-image photo, so the chip supplies its own ground.
   SAFE-ZONE: left-set + fit-content so it clears Батьківщина-мати at every width —
   on desktop it rides the left copy column (monument ~58% x, centre); on phones
   the monument is nudged RIGHT (--bgx:38%), so a left, narrow chip stays clear of
   the silhouette. STATIC — no motion — so it reads with JS off and needs no
   reduced-motion branch of its own. */
.v36 .hz8-proof{display:inline-flex;align-items:center;gap:13px;margin-top:26px;max-width:100%;
  padding:12px 17px;border-radius:15px;
  background:rgba(6,14,38,.72);border:1.5px solid rgba(255,255,255,.18);
  backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px);
  box-shadow:0 14px 34px -16px rgba(4,11,32,.7),0 1px 0 0 rgba(255,255,255,.06) inset;}
.v36 .hz8-proof-num{font-family:var(--font-display);font-weight:800;color:var(--yellow);
  font-size:clamp(2.1rem,3.3vw,2.65rem);line-height:1;letter-spacing:-.03em;white-space:nowrap;
  text-shadow:0 1px 0 rgba(70,44,0,.5),0 2px 12px rgba(6,16,44,.5);}
.v36 .hz8-proof-side{display:flex;flex-direction:column;gap:2px;min-width:0;}
.v36 .hz8-proof-unit{font-family:var(--font-display);font-weight:700;color:#FFFFFF;
  font-size:.92rem;line-height:1.2;text-shadow:0 1px 8px rgba(4,11,32,.6);}
.v36 .hz8-proof-meta{font-size:.82rem;color:#DBE6FA;line-height:1.3;text-shadow:0 1px 8px rgba(4,11,32,.6);}
.v36 .hz8-proof-claim{font-size:.78rem;color:#AFC2E6;line-height:1.3;text-shadow:0 1px 8px rgba(4,11,32,.6);}
@media (max-width:620px){
  .v36 .hz8-proof{margin-top:16px;padding:10px 14px;gap:11px;}
  .v36 .hz8-proof-num{font-size:2rem;}
  .v36 .hz8-proof-unit{font-size:.86rem;}
  .v36 .hz8-proof-meta{font-size:.78rem;}
}
...
export function HeroProspektP1() {
  const ref = useRef<HTMLElement>(null);
  const c = COPY;
  // Lead with the hook + the personalization clause only; the third sentence
  // («…офіційних питань…», the bank stat) repeats the figure the in-hero proof
  // strip now owns, so it's dropped here — every rendered word stays verbatim.
    ...
    <section className="hz8" ref={ref}>
      ...
      <div className="wrap">
        <div className="hz8-copy">
          ...
          {/* ── the proof, MERGED into the hero (no band below): ONE figure at
             primary weight + the subordinate detail + honest official-bank
             register, all VERBATIM from ./copy's shared constants. Static. */}
          <div className="hz8-proof">
            <b className="hz8-proof-num num">{P1_PROOF.bank}</b>
            <span className="hz8-proof-side">
              <span className="hz8-proof-unit">{P1_PROOF.bankUnit}</span>
              <span className="hz8-proof-meta">{P1_PROOF.detail}</span>
              <span className="hz8-proof-claim">{P1_PROOF.claim}</span>
            </span>
          </div>
        </div>
      </div>
