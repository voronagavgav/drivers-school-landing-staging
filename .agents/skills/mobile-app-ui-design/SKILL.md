...
Platform-specific craft layer. The user's brief always wins and sets the aesthetic direction — this skill grounds MOBILE execution of that direction (thumb zone, grids, tap targets, flows). `ui-ux-pro-max` is a lookup database; shadcn supplies structure to restyle; GSAP (`gsap-skills:*`) is the installed motion library for orchestrated/scroll-driven sequences.
...
#### Visual Cues & Imagery
- Use icons (SVG, never emoji-as-icons), illustrations, and images to make information digestible
...
- Include small icons or illustrations alongside options for personality (SVG, not emoji)
...
When building these designs (React/Next or HTML):
- Use Tailwind CSS utility classes for spacing, colors, and typography
- Import Lucide React for clean, consistent iconography
- Pick the charting approach per design — don't default to a library the project doesn't use
- Apply CSS transitions for micro-interactions and state changes; GSAP for orchestrated sequences
...
These guidelines are direction, not ritual. If you are a stronger model and your instinct is already specific to this product and platform, execute — spend the effort on refinement. The anti-patterns and the user's brief always still apply.
